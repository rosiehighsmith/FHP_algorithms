class Image
  # This program looks for 1's and changes numbers around it to 1
  ...
  def transform
    # look at each row(subarray) in the array
    @array.each_with_index do |subarray, r|
      # puts "r: " + r.to_s
      # puts subarray.inspect
      # within each row(subarray) look at each value in the columns
      subarray.each_with_index do |value, c|
        if value == 1
          ...
          # set value to the right to 1 if it's in a spot less than length of row
          @final_array[r][c+1] = 1 if c+1 <= subarray.length-1
          # set value below to 1 if it's not in a spot outside length of # of rows
